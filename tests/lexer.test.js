...
function runTests() {
    ...
    if (assertEquals(test1, expected1, 'Should recognize keywords and resolutions'))
        passed++;
    else
        failed++;
    ...
    if (assertEquals(test2, expected2, 'Should recognize identifiers with hyphens'))
        passed++;
    else
        failed++;
    // Test 3: Numbers
    ...
    if (assertEquals(test3, expected3, 'Should recognize keywords and numbers'))
        passed++;
    else
        failed++;
    // Test 4: Strings
    ...
    if (assertEquals(test4, expected4, 'Should recognize strings'))
        passed++;
    else
        failed++;
    ...
    if (assertEquals(test5, expected5, 'Should handle multiple lines'))
        passed++;
    else
        failed++;
    // Test 6: Resolutions
    ...
    if (assertEquals(test6, expected6, 'Should recognize resolutions'))
        passed++;
    else
        failed++;
    // Test 7: All keywords
    const test7 = lexer('resize input fps output encode bitrate audio watermark thumbnail');
    if (test7.length === 9 && test7.every((t) => t.type === 'KEYWORD')) {
    ...
    if (assertEquals(test8, expected8, 'Should recognize multiple hyphenated identifiers'))
        passed++;
    else
        failed++;
    ...
    if (assertEquals(test9, expected9, 'Should skip empty lines'))
        passed++;
    else
        failed++;
    // Summary
